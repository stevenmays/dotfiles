...
3. **Read full file contents for every changed file** — not just the touched hunks. Whole-file context is required to judge file size, structural drift, and boundary leaks. For large changesets, dispatch an `Explore` subagent with `model: "sonnet"` to collect file contents instead of pulling them all into the main context.

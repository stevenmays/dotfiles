...
2. **Gather the diff**: Run `git diff <base>...HEAD` and `git diff --name-only <base>...HEAD`. If the diff is empty, report that and stop.
   Also read `.claude/standards.md` if it exists — its rules are additional review criteria on top of the rubric; cite the specific rule when flagging a violation.
